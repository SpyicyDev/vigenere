...
module Ngram = struct
  ...
  let ngram file =
    ...
    let map =
      StringMap.map
        ...
        map
    in
    { ngrams = map; length = l; num = n; floor }
  ...
  let score ngram text =
    ...
    let rec aux score i =
      ...
      | false -> (
          let slice =
            String.uppercase_ascii @@ String.sub (String.concat "" @@ String.split_on_char ' ' text) i ngram.length
          in
          match StringMap.mem slice ngram.ngrams with
          | true -> aux (score +. StringMap.find slice ngram.ngrams) (i + 1)
          | false -> aux (score +. ngram.floor) (i + 1))
    in
    aux 0. 0
end
